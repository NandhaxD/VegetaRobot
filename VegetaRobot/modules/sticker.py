...
from VegetaRobot import telethn as bot, telethn as tbot
from VegetaRobot.events import register as asux, register
...
async def _(event):
    ...
    im1 = Image.open("VegetaRobot/resources/blank_background.png")
    if ik.endswith(".tgs"):
...
async def drawText(image_path, text):
    ...
    if lower_text:
        for l_text in textwrap.wrap(lower_text, width=15):
            u_bbox = draw.textbbox((0, 0), l_text, font=m_font)
            u_width = u_bbox[2] - u_bbox[0]
            u_height = u_bbox[3] - u_bbox[1]
...
× /getvideo: get video
× /tiny: make a sticker tiny
× /mmf: write in the sticker
