...
class KigyoTelegramHandler:
        ...
        def _message(func):
            try:
                if can_disable:
                    ...
                    self._dispatcher.add_handler(
                        MessageHandler(pattern, fun), group
                    )
    ...
    def callbackquery(self, pattern: str = None, bool = True):
        def _callbackquery(func):
            self._dispatcher.add_handler(CallbackQueryHandler(pattern=pattern, callback=func))
            log.debug(f'[KIGCALLBACK] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}')
            return func

        return _callbackquery

    def inlinequery(self, pattern: Optional[str] = None,  bool = True, pass_user_data: bool = True,
                    pass_chat_data: bool = True, chat_types: List[str] = None):
        def _inlinequery(func):
            self._dispatcher.add_handler(
                InlineQueryHandler(pattern=pattern, callback=func, pass_user_data=pass_user_data,
